...
import 'package:dana/presentation/widgets/header_user_widget.dart';
...
class CvUserWidget extends GetView<CvUserController> {
  String parameters='';
  CvUserWidget({this.parameters}){
  ...
  Widget build(BuildContext context) {
    return
     Scaffold(
       appBar: AppBar(
         title: Obx(()=>
         controller.getCvState.value == StateStatus.SUCCESS?
             Text(controller.cvEntity.value.data.name+' '+controller.cvEntity.value.data.family):Text('')),
         leading: IconButton(
           ...
           onPressed: (){
             ...
             controller.getCv(controller.popFromStack());
           },
         ),
       ),
       body:
           ...
           child: Column(
             children: [
               HeaderUserWidget(
                 ...
                 personalCode: controller.cvEntity.value.data.personal_code,
               ),
               ...
               SizedBox(height: space,),
               AboutMeWidget(aboutMe: controller.aboutMe.value,parentAction: editAboutMe,isUser: true,),
               SizedBox(height: space,),
               JobInformationWidget(experiences: controller.experienceLength>0?controller.experiences:[],removeExperience: removeExperience,isUser: true,),
               SizedBox(height: space,),
               EducationalInformationWidget(educations: controller.educationsLength>0?controller.educations:[],removeEducation: removeEducation,editEducation: editEducation,isUser: true,),
               SizedBox(height: space,),
               EducationCoursesWidget(courses:controller.coursesLength>0? controller.courses:[],removeCourse: removeCourse,editCourse: editCourse,isUser: true,),
               SizedBox(height: space,),
               SkillsWidget(skills: controller.skillsLength>0?controller.skills:[],removeSkill: removeSkill,isUser:true),
               SizedBox(height: space,),
               LanguageWidget(languages: controller.languagesLength>0?controller.languages:[],removeLanguage: removeLanguage,isUser:true),
               SizedBox(height: space,),
               ProjectsWidget(projects: controller.projectsLength>0?controller.projects:[],removeProject: removeProject,isUser:true),
               SizedBox(height: space,),
